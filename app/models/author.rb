class Author < ActiveRecord::Base
  ...
  def affiliations
    authorships.select {|a| a.position == 1}.sort {|a, b| b.article.pubdate <=> a.article.pubdate}.reject {|a| a.article.affiliation.empty?}.map {|a| [a.article.pubdate.year, a.article.affiliation]}.uniq
  end
end
